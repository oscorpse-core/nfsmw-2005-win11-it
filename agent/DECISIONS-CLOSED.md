# DECISIONS-CLOSED — do not reopen

Unless the user **explicitly** asks to reopen a topic, do not propose these again.

| Topic | Decision | Why |
|-------|----------|-----|
| Pink rivals in Car Lot | **REJECTED** | Liveries via mod; pink only via S1 markers |
| Visual day-one (pre-Sonny) | **REJECTED** | Shop visual max post-#15 + $0 is enough |
| Model B (no marker UI) | **REJECTED** | S1 (count=6) is enough |
| A2 early mid perf unlock | **REJECTED** | Perf unlock bands stay vanilla |
| UnlockAllThings = 1 | **REJECTED** | Stays **0** |
| Lot rank via TORQUE/MASS | **REJECTED** | Replaced by FE-bars |
| Infinite Junkman shop | **REJECTED** | A3 = finite 7×2 from markers |
| Community car pack | **OUT OF SCOPE** | Separate project |
| Publish QoL in public Release | **NOT NOW** | v1.0.1 stays stock; QoL local `-qol` + docs |
| Keep 3 unique HUD marker-gated | **KEEP** | Shop visual max ≠ unlock marker HUD uniques |

Source (IT): [`../qol-plan/PLAN.md`](../qol-plan/PLAN.md), [`../qol-plan/knowledge/11-COHERENCE-FREEZE.md`](../qol-plan/knowledge/11-COHERENCE-FREEZE.md), [`07-TECH-MODEL-B.md`](../qol-plan/knowledge/07-TECH-MODEL-B.md).
